...
class PSUM(nn.Module):
    ...
    def forward(self, layers, attention_mask, return_list= False):
        ...
        output = torch.zeros_like(layers[0])
        for i in range(self.config.count):
            logits = self.pre_layers[i](layers[-i-1], attention_mask)[0]
            logitses.append(logits)
        if return_list:
            return logitses
        avg_logits = torch.sum(torch.stack(logitses), dim=0) / self.config.count
...
class CodeBERTHSUMForSequenceClassification(RobertaPreTrainedModel):
    def __init__(self, config):
        ...
        if self.config.mix_type == 'HSUM':
            self.mixlayer = HSUM(config)
        else:
            self.mixlayer = PSUM(config)
        # Initialize weights and apply final processing
        self.post_init()
...
class CodeBERTMixFeaturesForSequenceClassification(RobertaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
        ...
        self.roberta = RobertaModel(config, add_pooling_layer=False)
        self.classifier = CustomRobertaClassificationHead(config, 4*768)
    ...
    def forward(
        ...
        sequence_output = torch.cat((layers[-1][:,0, ...],layers[-2][:,0, ...], layers[-3][:,0, ...], layers[-4][:,0, ...]),-1)
